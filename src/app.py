import streamlit as st
import random
import json
import torch
from model.model import NeuralNet
from utils.nltk_utils import bag_of_words, tokenize
import nltk
nltk.download('punkt')

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

with open('intents.json', 'r') as f:
    intents = json.load(f)

FILE = r"C:\Users\user\Desktop\ChatBot-with-Pytorch-master\data.pth"
data = torch.load(FILE)

input_size = data["input_size"]
hidden_size = data["hidden_size"]
output_size = data["output_size"]
all_words = data["all_words"]
tags = data["tags"]
model_state = data["model_state"]

model = NeuralNet(input_size, hidden_size, output_size).to(device)
model.load_state_dict(model_state)
model.eval()

bot_name = "Assistant:"


def chat():
    st.title("Hotel Assistant ChatBot")
    st.markdown("Type a message and press Enter to chat. Type 'exit' to quit.")

    user_input = st.text_input("You:", key="user_input")
    if user_input:
        # if user_input.lower() == "exit":
        #     st.text("Goodbye! Have a great day.")
        #     st.stop()

        sentence = tokenize(user_input)
        X = bag_of_words(sentence, all_words)
        X = X.reshape(1, X.shape[0])
        X = torch.from_numpy(X).to(device)

        output = model(X)
        _, predicted = torch.max(output, dim=1)
        tag = tags[predicted.item()]

        
        probs = torch.softmax(output, dim=1)
        prob = probs[0][predicted.item()]

        if prob.item() > 0.75:
            for intent in intents["intents"]:
                if tag == intent["tag"]:
                    st.text(f"{bot_name} {random.choice(intent['responses'])}")
        else:
            st.text(f"{bot_name} I do not understand...")


if __name__ == "__main__":
    chat()
